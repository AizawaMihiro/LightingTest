...
namespace Direct3D
{
	...
	DirectX::XMFLOAT4 lightPos = { 0.0f, -0.5f, -2.0f, 1.0f }; //ライトの位置
}
...
HRESULT Direct3D::InitNormalShader()
{
    ...
    hr = pDevice->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &(shaderBundle[SHADER_NORMALMAP].pVertexShader));
    if (FAILED(hr))
    {
    ...
    hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL, &(shaderBundle[SHADER_NORMALMAP].pPixelShader));
    if (FAILED(hr))
    {
...
HRESULT Direct3D::InitOutlineShader()
{
    ...
    hr = pDevice->CreateInputLayout(layout, 3, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &(shaderBundle[SHADER_OUTLINE].pVertexLayout));
    if (FAILED(hr))
    {
        MessageBox(nullptr, L"頂点インプットレイアウトの作成に失敗しました", L"エラー", MB_OK);
        return hr;
